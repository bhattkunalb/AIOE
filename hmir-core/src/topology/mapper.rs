...
impl CostMatrix {
    ...
    pub fn set_bandwidth(&mut self, from: &str, to: &str, gbps: f64) {
        self.bandwidths
            .insert((from.to_string(), to.to_string()), gbps);
    }
...
mod tests {
    ...
    fn test_latency_routing_decision() {
        ...
        assert!(
            !should_route_heavy_transfer,
            "Should stay on CPU when transfer penalty is higher than compute saving."
        );
    }
    ...
    fn test_battery_intent_override() {
        ...
        let latency_route = Router::should_route_to_accelerator(
            ...
            &cpu,
            &matrix,
            &ComputeIntent::Latency,
        );
        assert!(
            !latency_route,
            "Latency intent stays on CPU because NPU is slower."
        );
        ...
        let battery_route = Router::should_route_to_accelerator(
            ...
            &cpu,
            &matrix,
            &ComputeIntent::Battery,
        );
        assert!(
            battery_route,
            "Battery intent should override and push to NPU."
        );
    }
}

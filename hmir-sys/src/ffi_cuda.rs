...
extern "C" {
    ...
    pub fn cudaMalloc(devPtr: *mut *mut c_void, size: c_ulonglong) -> CudaError;
...
/// # Safety
///
/// - `vram_ptr` must point to a valid CUDA device allocation of at least `bytes` length.
/// - `ram_ptr` must point to a valid host allocation of at least `bytes` length.
/// - The caller must ensure no concurrent access to the memory regions during the swap.
